...
def plot_shared(all_data, params, best_loss):
    ...
    # Sparse majors (≈4) and unlabeled minor ticks
    from matplotlib.ticker import FixedLocator, LogLocator, NullFormatter
    import math
    pattern = [1, 2, 3, 5]
    exp_lo, exp_hi = int(math.floor(math.log10(ymin))), int(math.ceil(math.log10(ymax)))
    candidates = [m * 10**e for e in range(exp_lo, exp_hi + 1) for m in pattern if ymin <= m * 10**e <= ymax]
    majors = [candidates[i] for i in np.linspace(0, max(len(candidates)-1, 0), 4).round().astype(int)] if len(candidates) > 4 else candidates
    majors = sorted(set(majors))
    ax.yaxis.set_major_locator(FixedLocator(majors))
    ax.yaxis.set_minor_locator(LogLocator(base=10.0, subs=np.arange(2, 10) * 0.1))
    ax.yaxis.set_minor_formatter(NullFormatter())
    ax.set_yticklabels([f"{v:.1f}" if v < 10 else f"{v:.0f}" for v in majors])
    ...
    ax.grid(False)
    plt.tight_layout()
    out_plot = os.path.join(OUTPUT_DIR, "qint_log.png")
...
# -------- Main --------
def main():
    json_path = os.path.join(OUTPUT_DIR, "qint_log.json")
